...
<logic:iterate id="questionBody" name="subQuestion" property="prePresentation" indexId="indexQuestion">
	...
	<% if (((String)questionLabel).startsWith("image/")){%>
		...
		<html:img align="absmiddle" src="<%= request.getContextPath() + "/oldOnlineTests/testsManagement.do?method=showImage&amp;exerciseCode=" + questionCode+"&amp;imgCode="+index.toString() +"&amp;imgType="+questionLabel.toString()+"&amp;item="+item.toString()+"&amp;metadataCode="+metadataCode%>" altKey="questionLabel" bundle="IMAGE_RESOURCES"/>
...
<logic:iterate id="questionBody" name="subQuestion" property="presentation" indexId="indexQuestion">
	...
	<% if (((String)questionLabel).startsWith("image/")){%>
		<bean:define id="index" value="<%= (new Integer(Integer.parseInt(index)+1)).toString() %>"/>
		<html:img bundle="HTMLALT_RESOURCES" altKey="img.img" align="absmiddle" src="<%= request.getContextPath() + "/oldOnlineTests/testsManagement.do?method=showImage&amp;exerciseCode=" + questionCode+"&amp;imgCode="+index.toString() +"&amp;imgType="+questionLabel.toString()+"&amp;item="+item.toString()+"&amp;metadataCode="+metadataCode%>"/>
		<logic:equal name="imageLabel" value="true">
			</td><td>
		</logic:equal>
	<% } else if (((String)questionLabel).equals("image_label")){%>		
...
<logic:iterate id="optionBody" name="questionOption" property="optionContent" indexId="optionBodyIndex">
	...
	<% if (((String)optionLabel).startsWith("image/")){ %>
		<bean:define id="index" value="<%= (new Integer(Integer.parseInt(index)+1)).toString() %>"/>
		<html:img align="absmiddle" src="<%= request.getContextPath() + "/oldOnlineTests/testsManagement.do?method=showImage&amp;exerciseCode="+ questionCode +"&amp;imgCode="+index.toString() +"&amp;imgType="+optionLabel.toString()+"&amp;item="+item.toString()+"&amp;metadataCode="+metadataCode%>"  altKey="optionLabel" bundle="IMAGE_RESOURCES"/>
		...
		<logic:notEmpty name="rp" property="feedback">
			...
			<logic:iterate id="feedback" name="rp" property="feedback">
				...
				<%if (((String)feedbackLabel).startsWith("image/")){%>
					...
					<html:img align="absmiddle" src="<%= request.getContextPath() + "/oldOnlineTests/testsManagement.do?method=showImage&amp;exerciseCode=" + questionCode+"&amp;imgCode="+feedbackIndex.toString() +"&amp;feedbackCode="+rpIndex+"&amp;imgType="+feedbackLabel.toString()+"&amp;item="+item.toString()+"&amp;metadataCode="+metadataCode%>" altKey="feedbackLabel" bundle="IMAGE_RESOURCES"/>
					<logic:equal name="imageLabel" value="true">
						</td><td>
					</logic:equal>
